...
function calculateDistributionData(
  ...
  // Step 1: Process Max (Maximum value in data)
  const max = Math.max(...data);
  
  // Step 2: Process Min (Minimum value in data)
  const min = Math.min(...data);
  
  // Step 3: Process Width (Max - Min)
  const range = max - min;
  
  // Step 4: No of Data Points
  const dataPointCount = data.length;
  
  // Step 5: No of Classes (√ of data points)
  const binCount = Math.ceil(Math.sqrt(dataPointCount));
  
  // Step 6: Class Intervals (Process Width / No of Classes)
  // => Consider this value in three digits as shown in the example
  const binWidth = Number((range / binCount).toFixed(3));
  
  // Calculate Step A: minimum between (Process Min & LSL)
  const binStart = Math.min(min, lsl);

  // Calculate mean and standard deviation for control limits
  const mean = calculateMean(data) ?? 0;
  const stdDev = calculateStdDev(data, mean) ?? 0;
  
  // Calculate control limits (not specification limits)
  const ucl = mean + 3 * stdDev;
  const lcl = mean - 3 * stdDev;

  const bins = Array(binCount).fill(0);
  // Format bin edges with 3 decimal places as shown in the image
  const binEdges = Array(binCount + 1).fill(0).map((_, i) => 
    Number((binStart + i * binWidth).toFixed(3))
  );
  ...
  const histData = bins.map((count, i) => ({
    x: Number((binEdges[i] + binWidth / 2).toFixed(3)),
    y: count,
    binRange: `${binEdges[i].toFixed(3)}-${binEdges[i+1].toFixed(3)}`
  }));
  ...
  return {
    ...
    stats: {
      ...
      target: (usl + lsl) / 2,
      binEdges,
      // Include calculated control limits
...
export function calculateAnalysisData(
  ...
  // Add this line to filter out negative values if you want to match your Excel formula
  const nonNegativeMeasurements = measurements.filter(value => value >= 0);
  ...
  const mean = calculateMean(nonNegativeMeasurements);
  const stdDev = calculateStdDev(nonNegativeMeasurements, mean);
  ...
  return {
    metrics: {
      ...
      stdDevOverall: Number(stdDev.toFixed(6)),
      stdDevWithin: Number(withinStdDev.toFixed(6)),
      avgRange: Number(avgRange.toFixed(6)),
      ...
      target: Number((lsl + usl) / 2), // Calculate target as midpoint between LSL and USL
    },
    controlCharts: {
      xBarData,
      rangeData,
